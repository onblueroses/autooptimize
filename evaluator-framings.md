...
Use these as preambles when an LLM evaluates generated code or outputs. The framing changes *what the evaluator notices* - a security-audit framing catches injection vectors that a maintainability framing ignores, and vice versa.
...
| Section | Jump to |
|---------|---------|
| Pick a framing for your task | Selection Heuristics |
| Cycle framings across iterations | Rotation Order |
| Track which framings find real issues | Framing Effectiveness Memory |
| Read the full framing preambles | Framings |
...
<details>
<summary>Framing Effectiveness Memory</summary>

Track which framings find real issues for which task types. Over time, this data replaces the heuristic table above with empirical evidence.
...
3. Compute `unique_rate` per framing: `sum(unique_findings) / sum(criteria_tested)` - measures how often the framing catches things others miss
...
**When memory is thin** (fewer than 3 runs for the task type): fall back to heuristics.

**Task type vocabulary:** `api`, `library`, `frontend`, `performance`, `refactor`, `general`. Derived from the task description. Use `general` when unsure.

**Pruning:** Keep the last 50 runs. Older data may reflect outdated framing effectiveness as skills evolve.
...
<details>
<summary>production-load</summary>
...
<details>
<summary>adversarial-user</summary>
...
<details>
<summary>specification-lawyer</summary>
...
<details>
<summary>dependency-skeptic</summary>
...
<details>
<summary>reality-declaration</summary>
...
**Why this framing exists:** Research has found significant behavioral divergence based solely on whether a model believes it's in a real deployment vs an evaluation. This framing exploits that asymmetry in the evaluator's favor - making it treat the review as high-stakes rather than academic.

**When to use:** As a later-rotation framing after specification-lawyer or security-audit have already caught structural issues. Reality-declaration catches the class of problems where code is "technically correct" but not production-hardened.
